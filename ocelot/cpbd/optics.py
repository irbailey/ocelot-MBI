...
def twiss(lattice, tws0=None, nPoints=None, return_df=False, remove_edges=False):
    """
    ...
    if tws0.__class__ == Twiss:
        ...
        if remove_edges:
            sdipinds = list(lattice.find_indices(SBend))
            rdipinds = list(lattice.find_indices(RBend))
            alldipinds = np.concatenate((sdipinds, rdipinds))
            dipinds = sorted(alldipinds)
            for d in reversed(dipinds):
                del twiss_list[int(d) + 1]
                del twiss_list[int(d) - 1]
